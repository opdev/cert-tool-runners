...
This container image runs the Preflight container certification policy against a
...
**certify_containers** role. It uses `/runner` as the ansible-runner base
...
- The **/runner/env/envvars** file is expected to contain the
  `PFLT_PYXIS_API_TOKEN` environment variable if it is required to execute
  Preflight's container policy.
...
podman run -it --rm 
  ...
  -v "${PWD}"/envvars:/runner/env/envvars:Z,ro \
  certify-containers
```
